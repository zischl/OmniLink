...
void OmniCore::SwapInstanceLayout(int source, int dest) {
  if (AllInstances[DeviceMap(dest)].InstanceIP == NULL) {
    AllInstances[DeviceMap(dest)].InstanceIP =
        AllInstances[DeviceMap(source)].InstanceIP;
    strncpy(AllInstances[DeviceMap(dest)].InstanceName,
            AllInstances[DeviceMap(source)].InstanceName, OmniDevNameLen);
    strncpy(AllInstances[DeviceMap(dest)].IPv4_String,
            AllInstances[DeviceMap(source)].IPv4_String, 16);
    ...
    AllInstances[DeviceMap(source)].Edit(
        AllInstances[DeviceMap(dest)].InstanceName,
        AllInstances[DeviceMap(dest)].IPv4_String,
        AllInstances[DeviceMap(dest)].InstanceIP);
    AllInstances[DeviceMap(dest)].Edit(temp.InstanceName, temp.IPv4_String,
                                       temp.InstanceIP);
  }
}
...
void OmniCore::ScanInstances() {
  if (InstanceProbe->ScanState.load()) {
    std::unordered_map<uint32_t, std::string> AvailableInstances =
        *InstanceProbe->get();
    ...
    for (auto &[DevMapIDx, Instance] : AllInstances) {
      if (AvailableInstances.find(Instance.InstanceIP) ==
          AvailableInstances.end()) {
        Instance.Clear();
      }
    }
...
void OmniCore::Connect(char IP[16], char Auth[4]) {
  // ActiveInstances[static_cast<DeviceMap>(2)].InstanceSession = new
  // session(sessions.IOCP, ActiveInstances[Local].IPv4_String,
  // AllInstances[index].IPv4_String, 62485, 1450, ActiveInstances[index]);
}

void OmniCore::ConnectInstance(ConnectionRequest request) {
  ActiveInstances[request.DeviceID] =
      OmniActiveInstance(AllInstances[request.DeviceID].InstanceName,
                         AllInstances[request.DeviceID].IPv4_String,
                         AllInstances[request.DeviceID].InstanceIP);
  ActiveInstances[request.DeviceID].InstanceSession =
      new session(sessions.IOCP, AllInstances[DeviceMap::C0].IPv4_String,
                  AllInstances[request.DeviceID].IPv4_String, OmniPort, MTU,
                  &ActiveWindows);
  Logger::log(
      "Connecting to : ", ActiveInstances[request.DeviceID].InstanceName, "at ",
      ActiveInstances[request.DeviceID].IPv4_String);
  ActiveInstances[request.DeviceID].InstanceSession->OnIOCompletion =
...
void OmniCore::CreateStreamLink(WindowCreationData &WindowInfo) {
  ...
  HWND hwnd_cap =
      NewWindow->CreateWindowAsync(L"Test Window", hInstance, nCmdShow);
}

void OmniLink::ToggleFeature(FeatureTypes FeatureIndex,
                             DeviceMap Index = DeviceMap::C0) {
  switch (FeatureIndex) {
...
void OmniLink::ToggleWGC() {
  ...
  if (WGCStatus) {
    ...
    TransmitNetCommand(DeviceMap::L1, command, DeviceMap::L1,
                       OmniNet::Argonized);
    ...
    Nv = new NVENCODER();
    NvencSessionPtr = new NvencSession(D3D11Device, Nv->NVFunctions,
                                       WGSCapBuffer, 1920, 1080);

    AsynLink.StartSpinThread(OmniLink::WGCapSend,
                             ActiveInstances[DeviceMap::L1].InstanceSession,
                             WGSCapture, NvencSessionPtr);
...
void OmniLink::ToggleDDAPI() {
  if (DXGIStatus) {
    ...
    Nv = new NVENCODER();
    NvencSessionPtr =
        new NvencSession(D3D11Device, Nv->NVFunctions, DXGIBuffer, 1920, 1080);

    AsynLink.StartSpinThread(OmniLink::DXGICapSend,
                             ActiveInstances[DeviceMap::L1].InstanceSession,
                             DXGICap, NvencSessionPtr);

    // ExecuteCommand = &OmniLink::DXGICapSend;
  }
}

OmniCore::OmniCore(const HINSTANCE hInst, const int nCmdS)
    : hInstance(hInst), nCmdShow(nCmdS) {}
...
void OmniLink::OmniMain(HINSTANCE hInst, int nCmdS) {
  ...
  D3DDevice D3DDevStruct = Renderer.CreateD3d11Device(
      featureLevels, _countof(featureLevels), creationFlags);
  ...
  InstanceProbe = new Instances(ActiveInstances[DeviceMap::C0].InstanceName,
                                LocalIP, 62485);
  InstanceProbe->AwaitInstances([this]() { ScanInstances(); });
  ScanInstances();
...
void OmniLink::InitTrayIcon(HWND hwnd) {
  ...
  TrayIconData.hIcon =
      LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(OmniIcon));
...
LRESULT CALLBACK OmniLink::WProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) {

  OmniLink *omni =
      reinterpret_cast<OmniLink *>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
...
void D3D_BGRA2NV12_Compute() {
  ...
  YTextureDesc.BindFlags = D3D11_BIND_UNORDERED_ACCESS |
  D3D11_BIND_SHADER_RESOURCE; YTextureDesc.SampleDesc.Count = 1;
  ...
  UVTextureDesc.BindFlags = D3D11_BIND_UNORDERED_ACCESS |
  D3D11_BIND_SHADER_RESOURCE; UVTextureDesc.SampleDesc.Count = 1;
  UVTextureDesc.ArraySize = 1;
  UVTextureDesc.MipLevels = 1;

  D3D11Device->CreateTexture2D(&YTextureDesc, nullptr,
  PlaneYTexture.GetAddressOf()); D3D11Device->CreateTexture2D(&UVTextureDesc,
  nullptr, PlaneUVTexture.GetAddressOf());
  ...
  D3D11Device->CreateUnorderedAccessView(PlaneYTexture.Get(), &UAViewYDesc,
  UAViewY.GetAddressOf());
  ...
  D3D11Device->CreateUnorderedAccessView(PlaneUVTexture.Get(), &UAViewUVDesc,
  UAViewUV.GetAddressOf());
  ...
  hr = D3D11Device->CreateComputeShader(computeShaderBlob->GetBufferPointer(),
  computeShaderBlob->GetBufferSize(), nullptr, &NV12ComputeShader); if
  (FAILED(hr)) { OutputDebugString(L"\nCompute Shader Creation Failed
  Miserably");
  }
...
void lz4_compression(ID3D11DeviceContext *D3D11Context,
                     ID3D11Texture2D *stagingTexture,
                     ID3D11Texture2D *mainBuffer, unsigned int Width,
                     unsigned int Height) {
  ...
  std::thread t1([pixelResource, rowPitch, TotalChunkedBytes] {
    ...
    int compressedSize = LZ4_compress_default(
        reinterpret_cast<const char *>(chunk1.data()), compressedBuffer.data(),
        TotalChunkedBytes, maxCompressedSize);
    if (compressedSize <= 0) {
  ...
  std::thread t2([pixelResource, Height, rowPitch, TotalChunkedBytes] {
    std::vector<BYTE> chunk1(TotalChunkedBytes);
    memcpy(chunk1.data(), pixelResource + ((Height / 2) * rowPitch),
           TotalChunkedBytes);
    int maxCompressedSize = LZ4_compressBound(TotalChunkedBytes);
    std::vector<char> compressedBuffer(maxCompressedSize);
    int compressedSize = LZ4_compress_default(
