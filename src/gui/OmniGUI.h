...
public:
  ...
  void SetupImGui(HWND hwnd, ID3D11Device *D3D11Device,
                  ID3D11DeviceContext *D3D11Context, HANDLE *Events);
  ...
  inline void DeviceIconPreview(const ImVec2 &pos, const ImU32 &col,
                                const ImVec2 &text_size = ImVec2{0, 0},
                                const char *text = "") {
    DrawList->AddRect(ImVec2(pos.x - 50, pos.y - 40),
                      ImVec2(pos.x + 50, pos.y + 40), col, 5.0f, 0,
                      2.0f); // monitor
    DrawList->AddText(ImVec2(pos.x - (text_size.x * 0.5f), pos.y), col, text);
    DrawList->AddRect(ImVec2(pos.x, pos.y + 40), ImVec2(pos.x, pos.y + 55), col,
                      5.0f, 0, 2.0f); // handle
    DrawList->AddRect(ImVec2(pos.x - 15, pos.y + 55),
                      ImVec2(pos.x + 15, pos.y + 55), col, 10.0f, 0,
                      1.0f); // stand
  }

  inline void DeviceIcon(const char *label, const ImVec2 &pos,
    ...
    ImU32 col = hovered || held ? IM_COL32(128, 0, 255, 255)
                                : IM_COL32(255, 255, 255, 255);

    if (ImGui::BeginDragDropSource()) {
      ImGui::SetDragDropPayload("DeviceInfo", &(DeviceData->DevMapIndex),
                                sizeof(DeviceData->DevMapIndex));
      DeviceIconPreview(ImGui::GetCursorScreenPos(), col, text_size,
                        DeviceData->IPv4_String);
    ...
    DrawList->AddRect(ImVec2(pos.x - 50, pos.y - 40),
                      ImVec2(pos.x + 50, pos.y + 40), col, 5.0f, 0,
                      2.0f); // monitor
    DrawList->AddText(ImVec2(pos.x - (text_size.x * 0.5f), pos.y), col,
                      DeviceData->IPv4_String);
    DrawList->AddRect(ImVec2(pos.x, pos.y + 40), ImVec2(pos.x, pos.y + 55), col,
                      5.0f, 0, 2.0f); // handle
    DrawList->AddRect(ImVec2(pos.x - 15, pos.y + 55),
