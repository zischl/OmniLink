...
OmniCap::OmniCap(ActiveInstanceContainer &ActiveSessionList)
  ...
  if (WinCapHook == NULL && state == true) {
    WinCapHook =
        SetWinEventHook(EVENT_SYSTEM_MOVESIZESTART, EVENT_SYSTEM_MOVESIZEEND,
                        NULL, WinMvEventProc, 0, 0, WINEVENT_OUTOFCONTEXT);
  } else if (WinCapHook != NULL && state == false) {
    UnhookWinEvent(WinCapHook);
  }
}

void CALLBACK OmniCap::WinMvEventProc(HWINEVENTHOOK hWinEventHook, DWORD event,
...
bool OmniCap::GetEdgeProbeState() { return InputLinkStatus.load(); }
  ...
  std::thread EventThread([hwnd, MouseEventStatus, this]() {
    ...
    while (true) {
      ...
      while (MouseEventStatus->load()) {
        ...
        bool InsideCheck = (MouseX < ResWidth) && (MouseX > 0) &&
                           (MouseY < ResHeight) && (MouseY > 0);
...
void OmniCap::AddEdgeCondition(DeviceMap Index) {
  ...
  case DeviceMap::U1:
    ConditionManager.Add(
        Index, [&](int x, int y) { return y <= 0 && (x > 0 && x < ResWidth); });
    break;
  ...
  case DeviceMap::RU1:
    ConditionManager.Add(Index,
                         [&](int x, int y) { return x >= ResWidth && y <= 0; });
    break;

  case DeviceMap::LD1:
    ConditionManager.Add(
        Index, [&](int x, int y) { return x <= 0 && y >= ResHeight; });
    break;

  case DeviceMap::RD1:
    ConditionManager.Add(
        Index, [&](int x, int y) { return x >= ResWidth && y >= ResHeight; });
    break;
  }
}
...
void OmniCap::InputProcInit(LPARAM &lParam) {
  GetRawInputData((HRAWINPUT)lParam, RID_INPUT, NULL, &RawInputSize,
  ...
  ActiveSession->SessionSend(reinterpret_cast<CHAR *>(&MouseInput),
                             sizeof(INPUT), IOHeader);
...
void OmniCap::InputProcCallback(LPARAM &lParam) {
  ...
  GetRawInputData((HRAWINPUT)lParam, RID_INPUT, Buffer.data(), &RawInputSize,
                  sizeof(RAWINPUTHEADER));
  ...
  if (input->header.dwType == RIM_TYPEMOUSE) {
    ...
    ActiveSession->SessionSend(reinterpret_cast<CHAR *>(&KBInput),
                               sizeof(INPUT), IOHeader);
  }
}

void OmniCap::VoidExitCallback(LPARAM &lParam) { return; }
...
void OmniSynth::ProcInput(INPUT &input) { SendInput(1, &input, sizeof(INPUT)); }

void OmniSynth::ProcMouse(int x, int y) { SetCursorPos(x, y); }

void OmniSynth::ProcKey(INPUT &input) { SendInput(1, &input, sizeof(input)); }
