...
struct WindowCreationData {
  ...
  WindowCreationData(const char *Title, const size_t TitleLen, int w = 1920,
                     int h = 1080)
      : Width(w), Height(h) {
    SetTitle(Title, TitleLen);
  }

  WindowCreationData(const std::string &Title, const size_t TitleLen,
  ...
  const wchar_t *GetTitleW() {
    ...
    MultiByteToWideChar(CP_UTF8, 0, WindowName, -1, WCharName, NameLen);
...
using FuncArgTypes =
    std::variant<ArraySwapLayout, ConnectionRequest, WindowCreationData>;
...
struct OmniNetCommand {
  ...
  OmniNetCommand(CoreCommandsWArgs InCommandType, uint32_t InArgTypeIndex,
                 std::vector<uint8_t> InArgs)
      : CommandType(InCommandType), ArgTypeIndex(InArgTypeIndex),
        ArgArrayLength(static_cast<uint32_t>(InArgs.size())),
        Args(std::move(InArgs)) {}

  OmniNetCommand(CoreCommandsWArgs InCommandType, uint32_t InArgTypeIndex,
                 const uint8_t *InArgs, uint32_t InLength)
      : CommandType(InCommandType), ArgTypeIndex(InArgTypeIndex),
        ArgArrayLength(InLength), Args(InArgs, InArgs + InLength) {}
...
struct OmniCommand {
  ...
  OmniCommand(CoreCommandsWArgs InCommandType, uint32_t InArgTypeIndex,
              FuncArgTypes InArgs)
      : CommandType(InCommandType), ArgTypeIndex(InArgTypeIndex),
        Args(std::move(InArgs)) {}
...
/// <summary>
/// Custom Array Based Bounded Ring Buffer, Note that this queue will overwrite
/// if used beyond the limit. Used as a deque alternative with a max push limit
/// per drain.
/// </summary>
