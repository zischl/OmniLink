...
void Renderer::_createCustomBuffer(ComPtr<ID3D11Device> D3D11Device,
                                   ComPtr<ID3D11Texture2D>& customBuffer,
                                   int bufferWidth,
                                   int bufferHeight)
{
    ...
    HRESULT hr =
        D3D11Device->CreateTexture2D(&customBufferDesc, nullptr, customBuffer.GetAddressOf());
}
...
// Device DeviceStruct = Renderer.CreateD3d11Device(featureLevels, _countof(featureLevels),
// creationFlags);
D3DDevice Renderer::CreateD3d11Device(D3D_FEATURE_LEVEL(FeatureLevels)[],
                                      ...
                                      UINT& CreationFlags)
{
    ...
    HRESULT hr = D3D11CreateDevice(nullptr,
                                   ...
                                   FeatureLevelCount,
                                   D3D11_SDK_VERSION,
                                   &DeviceStruct.D3D11Device,
                                   &SelectedFeatureLevel,
                                   &DeviceStruct.D3D11Context);
    if (FAILED(hr)) {
...
ComPtr<IDXGISwapChain3> Renderer::CreateSwapChain(ID3D11Device* D3D11Device,
                                                  HWND& hwnd,
                                                  UINT Width,
                                                  UINT Height,
                                                  ...
                                                  SwapChainConfig SwapChainConfig)
{
    ...
    hr = Factory2->CreateSwapChainForHwnd(
        D3D11Device, hwnd, &swapchainConfig, nullptr, nullptr, &SwapChain);
    if (FAILED(hr)) {
...
std::vector<ComPtr<ID3D11Texture2D>> Renderer::GetSwapChainBuffersArray(IDXGISwapChain3* SwapChain,
                                                                        UINT Count)
{
...
ComPtr<ID3D11RenderTargetView> Renderer::CreateRTV(ID3D11Device* D3D11Device,
...
// not for dx 11
std::vector<ComPtr<ID3D11RenderTargetView>>
Renderer::CreateRTVArray(ID3D11Device* D3D11Device, IDXGISwapChain3* SwapChain, UINT Count)
{
...
ComPtr<ID3D11VertexShader> Renderer::CreateVertexShader(ID3D11Device* D3D11Device, LPCWSTR FileName)
{
    ...
    D3D11Device->CreateVertexShader(VertexShaderBlobTemp->GetBufferPointer(),
...
ComPtr<ID3D11InputLayout> Renderer::CreateInputLayout(ID3D11Device* D3D11Device,
...
ComPtr<ID3D11Buffer> Renderer::CreateIndexBuffer(ID3D11Device* D3D11Device,
                                                 const unsigned short (&Indices)[],
                                                 UINT ArraySize,
                                                 IndexBufferConfig BufferConfig)
{
