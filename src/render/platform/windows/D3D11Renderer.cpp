...
)
{
    ...
    if (RendererPtrStruct.D3D11Device == nullptr) {
        ...
        D3DDevice DeviceStruct =
            CreateD3d11Device(featureLevels, _countof(featureLevels), creationFlags);
    ...
    if (RendererPtrStruct.D3D11Context == nullptr) {
        GetDeferredContext(
            RendererPtrStruct.D3D11Device.Get(), RendererPtrStruct.D3D11Context.Get()
        );
    ...
    RendererPtrStruct.swapchain = CreateSwapChain(
        RendererPtrStruct.D3D11Device.Get(), hwnd, wdWidth, wdHeight, Factory2.Get()
    );
    ...
    RendererPtrStruct.renderTargetView =
        CreateRTV(RendererPtrStruct.D3D11Device.Get(), SwapBuffer.Get());
...
void D3D11Renderer::SetShaders(ID3D11DeviceContext* D3D11Context, HWNDxShaders* shaders)
{
    ...
    D3D11Context->IASetVertexBuffers(
        0,
        1,
        shaders->vertexBuffer.GetAddressOf(),
        &shaders->VertexBufferStride,
        &shaders->VertexBufferOffset
    );
