...
class ByteStream
{
  protected:
    ...
    {
        Dest = (static_cast<uint32_t>(Source[0]) << 24) | (static_cast<uint32_t>(Source[1]) << 16) |
    ...
    static inline void WriteU8(uint8_t Value, uint8_t* Dest) { Dest[0] = Value; }
...
class ByteStreamReader : private ByteStream
{
  public:
    ...
    ByteStreamReader(uint32_t DataLen, uint8_t* DataPtr) : CurrentLength(DataLen), Data(DataPtr) {}
...
class ByteStreamEx : private ByteStream
{
  public:
    ...
    explicit ByteStreamEx(uint32_t StartingLen, uint8_t* DataPtr)
        : CurrentLength(StartingLen), Data(DataPtr)
    {
    }
...
class ByteVecStreamEx : private ByteStream
{
  public:
    ...
    ByteVecStreamEx(uint32_t ExpectedLength) : CurrentLength(0) { Data.reserve(ExpectedLength); }
    ...
    void WriteU8String(const std::u8string_view& String);
